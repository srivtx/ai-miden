// server.js
//
// Project 36: The Tests
// ======================
// Vitest + supertest. We refactor the server slightly: the `app` and `db`
// are now exported, so test files can import them and use a fresh in-memory
// database. The HTTP listener is only started when this file is run directly
// (require.main === module). Tests use supertest(app) which talks to the
// express app without binding a port — much faster.
//
// Setup:
//   export STRIPE_SECRET_KEY=sk_test_... STRIPE_WEBHOOK_SECRET=whsec_...
//   redis-server
//   npm install ioredis rate-limiter-flexible node-cron bullmq ws y-websocket yjs stripe vitest supertest
//   node server.js
//
// Test:
//   npx vitest run
//
// Migration (unchanged from 35):
//   + subscriptions table
...
new Worker(
  ...
  async (job) => {
    if (job.name === "welcome") {
      await sendEmail({ to: job.data.email, subject: "Welcome!", text: `Welcome, ${job.data.username}!` });
    }
  },
  { connection: redis }
);
        ...
        if (userId) {
          await db("subscriptions")
            .insert({
              ...
              plan: "pro",
              status: "active",
              current_period_end: (Date.now() / 1000 + 30 * 24 * 3600) | 0,
              created_at: Date.now(),
            })
            .onConflict("user_id")
            .merge();
...
// PROJECT 36: db connection is now overridable for tests
const db = knex(
  process.env.DATABASE_URL
    ? { client: "better-sqlite3", connection: process.env.DATABASE_URL, useNullAsDefault: true }
    : { client: "better-sqlite3", connection: { filename: "app.db" }, useNullAsDefault: true }
);
...
async function migrate() {
  ...
  await db.schema.createTableIfNotExists("workspaces", (t) => {
    ...
    t.integer("owner_id").notNullable().references("id").inTable("users");
    t.bigInteger("created_at").notNullable();
  });
  await db.schema.createTableIfNotExists("workspace_members", (t) => {
    t.integer("workspace_id").notNullable().references("id").inTable("workspaces").onDelete("CASCADE");
    t.integer("user_id").notNullable().references("id").inTable("users").onDelete("CASCADE");
...
const memberUpdateSchema = z.object({ role: z.enum(["admin", "member", "guest"]) });
...
function authMiddleware(req, res, next) {
  ...
  catch (err) { return next(new UnauthorizedError("invalid or expired token")); }
}
...
app.get("/subscriptions/me", authMiddleware, asyncHandler(async (req, res) => {
  ...
  res.json({ plan: sub.plan, status: sub.status, currentPeriodEnd: sub.current_period_end ? new Date(sub.current_period_end * 1000).toISOString() : null });
}));
...
app.post("/webhooks", authMiddleware, validate(webhookCreateSchema), asyncHandler(async (req, res) => {
  ...
  const [id] = await db("webhooks").insert({ owner_id: req.user.userId, url, secret, events: JSON.stringify(events), active: 1, created_at: Date.now() });
  res.status(201).json({ id, url, events, secret, active: true, message: "Save the secret now. It will not be shown again." });
}));
app.get("/webhooks", authMiddleware, asyncHandler(async (req, res) => {
  const webhooks = await db("webhooks").where({ owner_id: req.user.userId }).select("id", "url", "events", "active", "created_at");
  res.json({ data: webhooks.map((w) => ({ ...w, events: JSON.parse(w.events) })) });
}));
app.patch("/webhooks/:id", authMiddleware, validate(webhookUpdateSchema), asyncHandler(async (req, res) => {
  const updates = req.validated;
  if (Object.keys(updates).length === 0) throw new ValidationError([{ path: "", message: "no fields to update" }]);
  const w = await db("webhooks").where({ id: req.params.id, owner_id: req.user.userId }).first();
...
app.post("/workspaces/:workspaceId/members", authMiddleware, requireRole("admin"), validate(memberAddSchema), asyncHandler(async (req, res) => {
  ...
  fireWebhook("workspace.member_added", { workspaceId: req.params.workspaceId, userId, role });
  res.status(201).json({ message: "Member added" });
}));
app.patch("/workspaces/:workspaceId/members/:userId", authMiddleware, requireRole("admin"), validate(memberUpdateSchema), asyncHandler(async (req, res) => {
  ...
  fireWebhook("workspace.role_changed", { workspaceId: req.params.workspaceId, userId: req.params.userId, role });
  res.json({ message: "Role updated" });
}));
app.get("/workspaces/:workspaceId/members", authMiddleware, requireRole("guest"), asyncHandler(async (req, res) => {
  const members = await db("workspace_members")
    ...
    .select("users.id", "users.username", "users.email", "workspace_members.role");
...
app.patch("/users/:id", authMiddleware, validate(userUpdateSchema), asyncHandler(async (req, res) => {
  ...
  fireWebhook("user.updated", { id: req.params.id, fields: Object.keys(updates) });
  const u = await db("users").select("id", "username", "email", "balance", "created_at").where({ id: req.params.id }).first();
  res.json(u);
}));
...
app.post("/sessions", validate(sessionCreateSchema), asyncHandler(async (req, res) => {
  ...
  if (!(await bcrypt.compare(password, u.hash))) throw new UnauthorizedError("invalid credentials");
...
app.post("/sessions/forgot", validate(forgotPasswordSchema), asyncHandler(async (req, res) => {
  ...
  if (u) {
    ...
    await db("users").where({ id: u.id }).update({ password_reset_token: h, password_reset_expires_at: Date.now() + 60 * 60 * 1000 });
    await emailQueue.add("password-reset", { email, token: t }, { attempts: 3, backoff: { type: "exponential", delay: 1000 } });
  }
...
app.post("/sessions/reset", validate(resetPasswordSchema), asyncHandler(async (req, res) => {
  ...
  if (!u || u.password_reset_expires_at < Date.now()) throw new ValidationError([{ path: "token", message: "invalid or expired token" }]);
  await db("users").where({ id: u.id }).update({ hash: await bcrypt.hash(password, 10), password_reset_token: null, password_reset_expires_at: null });
...
app.patch("/posts/:id", authMiddleware, validate(postUpdateSchema), asyncHandler(async (req, res) => {
  ...
  if (!p) throw new NotFoundError("Post not found");
  if (p.user_id !== req.user.userId) throw new ForbiddenError("You can only update your own posts");
...
app.delete("/posts/:id", authMiddleware, asyncHandler(async (req, res) => {
  const p = await db("posts").where({ id: req.params.id }).first();
  if (!p) throw new NotFoundError("Post not found");
  if (p.user_id !== req.user.userId) throw new ForbiddenError("You can only delete your own posts");
...
// PROJECT 36: export for tests
module.exports = { app, db, migrate, SECRET, userCreateSchema };

// PROJECT 36: only start the listener when run directly, not when imported by tests
  ...
  migrate().then(() => {
    ...
    subscriber.on("message", (channel, message) => {
      ...
      wss.clients.forEach((client) => {
        if (client.readyState === WebSocket.OPEN) client.send(JSON.stringify({ type: "presence", event }));
      });
    });
    ...
    wss.on("connection", async (ws, req) => {
      ...
      let user;
      try { user = jwt.verify(token, SECRET); } catch (err) { ws.close(1008, "Unauthorized"); return; }
      await setOnline(user.userId);
      ws.send(JSON.stringify({ type: "presence:list", users: await getOnlineUsers() }));
      ...
      ws.on("close", async () => { clearInterval(heartbeat); await setOffline(user.userId); });
    });
