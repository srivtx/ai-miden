...
// =========================
// Docker multi-stage build and docker-compose orchestration. The server code
// is simplified (no Yjs/WebSocket) to keep the Dockerfile clear. The real value
// is in the adjacent Dockerfile and docker-compose.yml.
//
// Setup:
//   docker-compose up -d
...
//   Stage 1 (build): npm ci --production=false + npm prune --production
//   Stage 2 (run): copy node_modules, run as non-root `node` user
...
app.get("/", (req, res) => res.json({ status: "healthy", uptime: process.uptime() }));
