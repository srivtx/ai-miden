...
// Project 31: The CRDT
// =====================
// Adds Yjs for co-editing. The HTTP server hosts the regular REST API.
// A second WebSocketServer attached to the same HTTP upgrade handler
// relays Yjs document updates between clients. Clients connect to
// ws://localhost:3000/<doc-name> and edit the same Y.Doc.
//
// Setup:
//   redis-server
...
//   node -e "const Y=require('yjs');const {WebsocketProvider}=require('y-websocket');
//     const ydoc=new Y.Doc();new WebsocketProvider('ws://localhost:3000','doc1',ydoc);
...
migrate().then(() => {
  ...
  // PROJECT 31: Yjs WebSocket relay (co-editing)
  const yWss = new WebSocketServer({ noServer: true });
  yWss.on("connection", setupYjs);

  // Route WebSocket upgrades: chat at /, Yjs at /<doc-name>
