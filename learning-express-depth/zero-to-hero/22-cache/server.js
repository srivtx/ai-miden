...
class ValidationError extends HttpError { constructor(issues) { super(400, 'VALIDATION', 'Validation failed'); this.issues = issues; } }
class UnauthorizedError extends HttpError { constructor(message = 'Unauthorized') { super(401, 'UNAUTHORIZED', message); } }
class ForbiddenError extends HttpError { constructor(message = 'Forbidden') { super(403, 'FORBIDDEN', message); } }
class NotFoundError extends HttpError { constructor(message = 'Not Found') { super(404, 'NOT_FOUND', message); } }
class ConflictError extends HttpError { constructor(message = 'Conflict') { super(409, 'CONFLICT', message); } }
...
app.get('/users/:id', asyncHandler(async (req, res) => {
  ...
  if (cached) { req.log.debug({ cacheKey }, 'cache hit'); return res.json(cached); }
...
app.post('/sessions/reset', validate(resetPasswordSchema), asyncHandler(async (req, res) => {
  ...
  if (!user || user.password_reset_expires_at < Date.now()) {
    throw new ValidationError([{ path: 'token', message: 'invalid or expired token' }]);
  }
  const hash = await bcrypt.hash(password, 10);
  await db('users').where({ id: user.id }).update({
    hash, password_reset_token: null, password_reset_expires_at: null,
  });
...
app.get('/posts', asyncHandler(async (req, res) => {
  ...
  const [posts, countResult] = await Promise.all([
    db('posts').join('users', 'posts.user_id', 'users.id').select('posts.*', 'users.username as author')
      .orderBy('posts.created_at', 'desc').limit(limit).offset(offset),
...
app.get('/posts/:id', asyncHandler(async (req, res) => {
  ...
  const cached = cache.get(cacheKey);
  if (cached) { req.log.debug({ cacheKey }, 'cache hit'); return res.json(cached); }
  const post = await db('posts').join('users', 'posts.user_id', 'users.id').select('posts.*', 'users.username as author')
