// server.js
//
// Project 34: The Webhooks
// =========================
// Outbound webhooks: a way to push events to external systems in real time.
// A third-party (CI runner, billing system, partner app) registers a URL and
// a list of events it cares about. When one of those events happens in our
// system, we POST a JSON payload to the registered URL, signed with HMAC-SHA256
// so the receiver can verify it came from us.
//
// Why not just let them poll? Because polling is slow, wasteful, and creates
// artificial latency. A webhook arrives within milliseconds.
//
// Why signed? Because the receiver needs to know it's actually us sending
// it. Without a signature, anyone who learns the URL could forge events.
//
// Setup:
//   redis-server
...
//   node server.js
//
// Migration (added to migrate()):
//   CREATE TABLE webhooks (id, owner_id, url, secret, events TEXT, active, created_at)
//     events = JSON-encoded array of event names
...
const webhookDeliveryWorker = new Worker(
  ...
  async (job) => {
    ...
    const sig2 = crypto.createHmac("sha256", secret).update(signed).digest("hex");
    ...
    try {
      res = await fetch(url, {
        ...
        headers: {
          ...
          "X-Webhook-Signature": `t=${ts},v1=${sig2}`,
...
webhookDeliveryWorker.on("failed", (job, err) => {
  logger.error(
    { err: err.message, url: job?.data?.url, attempt: job?.attemptsMade },
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    const w = await db("webhooks")
      ...
      .first();
    if (!w) throw new NotFoundError("Webhook not found");
    await fireWebhook("ping", { sentBy: req.user.userId });
